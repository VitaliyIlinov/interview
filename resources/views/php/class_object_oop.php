...
<p>
    ...
    определить класс,
    как абстрактный, необходимо записать ключевое слово abstract перед словом class.<br>
...
<p>
    ...
    Например, у нас есть
    геометрическая фигура и класс Figure. Поскольку, мы еще не знаем, какая именно фигура, то мы не можем ее себе
    представить или нарисовать. Это значит что создавать экземпляр этого класса не имеет смысла и мы можем определить
    соответствующий класс как абстрактный, а классы Triangle, Square, CIrcle как дочерние классы геометрической фигуры.
</p>
<p>
    В абстрактном классе допускается объявление методов без тела самих методов. Такие методы называются абстрактными .
    Это означает, что этот метод обязательно должен быть реализован в дочернем классе. Иначе это приведет
    к ошибке. Для того, чтобы определить метод, как абстрактный, также необходимо указать ключевое слово abstract перед
    определением метода
</p>
...
<h2>Интерфейсы</h2>
<p>
    <b><em>Интерфейсы</em></b> - это структуры, которые описывают то, какие константы и методы должен содержать класс,
    который будет
    реализовывать интерфейс. При этом интерфейс не должен содержать реализацию указанных методов. Он содержит просто
    перечисление того, что должно быть в классе (прототипы функций).
</p>
<p>
    Интерфейс определяется при помощи ключевого слова <b><em>interface</em></b>.
</p>
<p>
    Для реализации интерфейса, необходимо записать ключевое слово <b><em>implements</em></b> в определении класса и
    указать название интерфейса.
</p>
<p>
    В этом случае класс обязательно должен содержать реализацию всех методов, перечисленных в интерфейсе. При этом,
    сигнатура методов должна полностью совпадать с сигнатурой, указанной в интерфейсе. Иначе возникнет fatal error.
    Важным свойством интерфейсов является то, что один класс может реализовывать несколько интерфейсов.
</p>
<p>
    Интерфейсы поддерживают наследование. Для этого также используется ключевое слово <b><em>extends</em></b>. Отметим,
    что для интерфейсов поддерживается множественное наследование.
</p>
<p>
    <b><em> В чем отличие интерфейса от абстрактного класса?</em></b> Для интерфейсов, как и для абстрактных классов
    нельзя создать
    экземпляры. Отличие абстрактного класса от интерфейса состоит в том, что в интерфейсе необходимо опускать тело всех
    методов, а абстрактный метод может содержать реализацию отдельных методов. Также для классов невозможно
    множественное наследование, в отличии от интерфейсов.
</p>

<h2>Исключение</h2>
<p>
    <b><em>Исключение</b></em> - это действие, которое не укладывается в работу приложения. Исключения используют для того, чтобы
